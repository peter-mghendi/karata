...
public sealed class CurrentUserService(
    ...
    KarataPlatformContext platformContext,
...
)
{
    ...
    public async Task<User?> GetUserAsync(ClaimsPrincipal? principal = null, CancellationToken ct = default)
    {
        ...
        try
        {
            if (await platformContext.Users.FindAsync([userId], cancellationToken: ct) is { } user) return user;
            if (!_options.AutoProvisionEnabled) return null;

            user = platformContext.Users.Add(_options.NewUserFactory(principal!)).Entity;

            await platformContext.SaveChangesAsync(ct);
            return user;
        }
        finally
        {
