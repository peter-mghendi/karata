...
public sealed class RoomStreams : IDisposable
{
    ...
    public IObservable<(UserData user, int count)> MoveCardCountFromDeckToHand =>
        _moveCardCountFromDeckToHand.AsObservable();
    ...
    public IObservable<(UserData user, IReadOnlyList<Card> cards)> MoveCardsFromHandToPile =>
        _moveCardsFromHandToPile.AsObservable();
    ...
    internal void OnMoveCardsFromHandToPile(UserData u, IReadOnlyList<Card> c) =>
        _moveCardsFromHandToPile.OnNext((u, c));
